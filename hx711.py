...
class HX711:
    ...
    def __init__(self, dout_pin, pd_sck_pin, gain_channel_A=128, select_channel="A"):
        ...
        if isinstance(dout_pin, int):
            if isinstance(pd_sck_pin, int):
                ...
                raise TypeError(
                    "pd_sck_pin must be type int. "
                    "Received pd_sck_pin: {}".format(pd_sck_pin)
                )
        else:
            raise TypeError(
                "dout_pin must be type int. " "Received dout_pin: {}".format(dout_pin)
            )
        ...
        if readings > 0 and readings < 100:
            ...
            if result != False:
                if self._current_channel == "A" and self._gain_channel_A == 128:
                    ...
                    if self._debug_mode:
                        print(
                            "Cannot zero() channel and gain mismatch.\n"
                            "current channel: {}\n"
                            "gain A: {}\n".format(
                                self._current_channel, self._gain_channel_A
                            )
                        )
                    return True
            else:
    ...
    def set_offset(self, offset, channel="", gain_A=0):
        ...
        if isinstance(offset, int):
            ...
            raise TypeError(
                'Parameter "offset" has to be integer. '
                "Received: " + str(offset) + "\n"
            )
    ...
    def set_scale_ratio(self, scale_ratio, channel="", gain_A=0):
        ...
        if isinstance(gain_A, int):
            ...
            raise TypeError(
                'Parameter "gain_A" has to be integer. '
                "Received: " + str(gain_A) + "\n"
            )
    ...
    def set_data_filter(self, data_filter):
        ...
        if callable(data_filter):
            ...
            raise TypeError(
                'Parameter "data_filter" must be a function. '
    ...
    def set_debug_mode(self, flag=False):
        ...
        if flag == False:
            ...
            raise ValueError(
                'Parameter "flag" can be only BOOL value. ' "Received: {}".format(flag)
            )
    ...
    def _read(self):
        ...
        for _ in range(24):
            ...
            end_counter = time.perf_counter()
            if (
                end_counter - start_counter >= 0.00006
        ...
        if (
            data_in == 0x7FFFFF
            or data_in == 0x800000  # 0x7fffff is the highest possible value from hx711
    ...
    def get_last_raw_data(self, channel="", gain_A=0):
        ...
        if channel == "A" and gain_A == 128:
            ...
            raise ValueError(
                'Parameter "channel" has to be "A" or "B". '
                'Received: {} \nParameter "gain_A" has to be 128 or 64. Received {}'.format(
                    channel, gain_A
                )
            )
    ...
    def outliers_filter(self, data_list, stdev_thresh=1.0):
        ...
        # filter out booleans
        data = [
            num for num in data_list if (num != -1 and num != False and num != True)
        ]
        if not data:
            return []
